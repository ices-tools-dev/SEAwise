...
for (i in 1:length(FS)){
 ...
 # Model 1 
  ...
  res$coeff[i]=coefficients(lm(value~TOTFISHDAYS+0,data=Merg_temp2))
  res$points[i]=nrow(Merg_temp2)
  Merg_temp2$EnCosMod1=res$coeff[i]*Merg_temp2$TOTFISHDAYS
  
  res$RMSE[i]=sqrt(sum((as.numeric(Merg_temp2$value)-Merg_temp2$EnCosMod1)^2,na.rm=TRUE)/(length(Merg_temp2$value)+1)) 
...
write.table(res,"EnCost_model1.csv",sep=";",row.names=F)
...
for (i in 1:length(FS)){
  ...
  res$coeff[i]=coefficients(lm(value~FP_FD+0,data=Merg_temp3))
  res$points[i]=nrow(Merg_temp2)
  
  Merg_temp2$EnCosMod2=res$coeff[i]*Merg_temp3$FP*Merg_temp3$TOTFISHDAYS
  
  res$RMSE[i]=sqrt(sum((as.numeric(Merg_temp2$value)-Merg_temp2$EnCosMod2)^2,na.rm=TRUE)/(length(Merg_temp2$value)+1)) 
...
write.table(res,"EnCost_model2.csv",sep=";",row.names=F)
...
for (i in 1:length(FS)){
  ...
  # Model 1 
  Merg_temp2=Merg_temp[Merg_temp$FS==FS[i],]  
  res$coeff[i]=coefficients(lm(value~REVENUES+0,data=Merg_temp2))
  res$points[i]=nrow(Merg_temp2)
...
source(Disaggregation_of_the_costs_SECFISH)

Disaggr(DF,key_table,DF2,path=getwd())
