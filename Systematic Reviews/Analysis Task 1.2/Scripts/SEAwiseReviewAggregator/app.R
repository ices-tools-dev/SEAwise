...
ui <- fluidPage(
    ...
    hr(),
    print("This shiny webApp was developed by Elliot J. Brown based on data derived from the project SEAwise, funded by the European Union's HORIZON 2020 research and innovation programme under grant agreement No. 101000318.")
)
#====
    ...
    output$wp2TS <- renderPlot({
        ...
        if(nrow(wp2Data()) <= 6){ 
            wp2TSplot <- ggplot() +
                ...
                scale_x_continuous(n.breaks = max(2, round(((max(wp2Data()$Year, na.rm = T)-min(wp2Data()$Year, na.rm = T))/2), 0))) +
            ...
            wp3TSplot <- ggplot() +
                geom_bar(data = wp3Data(),
                         mapping = aes(x = Year,
                                       fill = WP),
                         position = position_dodge(preserve = "single")) +
                scale_x_continuous(n.breaks = max(2, round(((max(wp3Data()$Year, na.rm = T)-min(wp3Data()$Year, na.rm = T))/5), 0))) +
            ...
            wp4TSplot <- ggplot() +
                geom_bar(data = wp4Data(),
                         mapping = aes(x = Year,
                                       fill = WP),
                         position = position_dodge(preserve = "single")) +
                scale_x_continuous(n.breaks = max(2, round(((max(wp4Data()$Year, na.rm = T)-min(wp4Data()$Year, na.rm = T))/5), 0))) +
            ...
            wp5TSplot <- ggplot() +
                geom_bar(data = wp5Data(),
                         mapping = aes(x = Year,
                                       fill = WP),
                         position = position_dodge(preserve = "single")) +
                scale_x_continuous(n.breaks = max(2, round(((max(wp5Data()$Year, na.rm = T)-min(wp5Data()$Year, na.rm = T))/5), 0))) +
