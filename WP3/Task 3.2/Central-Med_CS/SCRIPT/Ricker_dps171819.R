...
case="Deep-water rose shrimp 17-1819"
var=read.csv('DPS171819_50-500.csv',sep=';',header=TRUE)
...
case="Deep-water rose shrimp GSAs 17-18-19"

##################################################################################


mod<-nls(R~ a*S*exp(-bin*S), start=list(a=ain) )
err<-residuals(mod)
...
legend_color<-"black"
  ...
  #botT
  mod<-nls(R~a*S*exp(-bin*S-c*botT), start=list(a=ain,c=cin),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  #sst
  mod<-nls(R~a*S*exp(-bin*S-c*sst), start=list(a=ain,c=cin),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  #nppv
  mod<-nls(R~a*S*exp(-bin*S-c*nppv), start=list(a=ain,c=cin),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  #botT-nppv
  mod<-nls(R~ a*S*exp(-bin*S-c*botT-d*nppv), start=list(a=ain,c=cin,d=din),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  #sst-nppv
  mod<-nls(R~ a*S*exp(-bin*S-c*sst-d*nppv), start=list(a=ain,c=cin,d=din),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  legend("topleft",legend=legend_names,col=legend_color,lwd=c(2,2),lty=legend_lty)
  
  
  tabella<-as.data.frame(tabella)
  colnames(tabella)<-c("Model","Formula","a","c","d","f","AIC","SSE","ME","MSE")
