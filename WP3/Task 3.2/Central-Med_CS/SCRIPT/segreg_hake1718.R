...
legend_color<-"black"
 ...
 # bin<-coeffs(mod)[2]
  ...
  # #botT
  # mod<-nls(R~a*pmin(S*exp(-c*botT),S_star), start=list(a=ain,c=cin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # 
  # #sst
  # mod<-nls(R~ a*pmin(S*exp(-c*sst),S_star), start=list(a=ain,c=cin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botso
  # mod<-nls(R~a*pmin(S*exp(-c*botso),S_star), start=list(a=ain,c=cin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botT-sst
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*sst),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botT-so
  mod<-nls(R~ a*pmin(S*exp(-c*botT-d*so),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botT-botso
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*botso),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botT-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*nppv),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #sst-so
   mod<-nls(R~ a*pmin(S*exp(-c*sst-d*so),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #sst-botso
  # mod<-nls(R~ a*pmin(S*exp(-c*sst-d*botso),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #sst-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*sst-d*nppv),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #so-botso
  # mod<-nls(R~a*pmin(S*exp(-c*so-d*botso),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #so-nppv
  # mod<-nls(R~a*pmin(S*exp(-c*so-d*nppv),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botso-nppv
  mod<-nls(R~ a*pmin(S*exp(-c*botso-d*nppv),S_star), start=list(a=ain,c=cin,d=din),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botT-sst-so
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*sst-f*so),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botT-sst-botso
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*sst-f*botso),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botT-sst-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*sst-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botT-so-botso
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*so-f*botso)), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #botT-so-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*botT-d*so-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # #botT-botso-nppv
  mod<-nls(R~ a*pmin(S*exp(-c*botT-d*botso-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  err<-residuals(mod)
  N_nona<-N-sum(length(which(is.na(err))))
  SSE=sum(err^2,na.rm=T)
  ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #sst-so-botso
  # mod<-nls(R~ a*pmin(S*exp(-c*sst-d*so-f*botso),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #sst-so-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*sst-d*so-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #sst-botso-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*sst-d*botso-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
  ...
  # # #so-botso-nppv
  # mod<-nls(R~ a*pmin(S*exp(-c*so-d*botso-f*nppv),S_star), start=list(a=ain,c=cin,d=din,f=fin),data=data_case )
  # err<-residuals(mod)
  # N_nona<-N-sum(length(which(is.na(err))))
  # SSE=sum(err^2,na.rm=T)
  # ME=sum(abs(err),na.rm=T)/N_nona
